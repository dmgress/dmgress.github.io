...
categories:
- M202 July 2014
date: 2014-08-11 20:51:07+00:00
excerpt: The fourth week covered fault tolerance and availability in MongoDB setups.
  It covered how to do a rolling maintenance and the aspects of network traffic to
  specific parts of replica sets and clusters.
meta:
  _edit_last: '6246633'
  _publicize_done_external: a:1:{s:7:"twitter";a:1:{i:89917968;b:1;}}
  _wpas_done_129558: '1'
  geo_public: '0'
  publicize_twitter_url: http://t.co/ZFM3JiocyS
  publicize_twitter_user: 123579D
parent_id: '0'
password: ''
permalink: /2014/08/11/m202-july-2014-fourth-week/
published: true
status: publish
tags:
- m202
- mongodb
title: M202 July 2014 Fourth week
type: post
---
...
3. issue a `[db.shutdownServer()](http://docs.mongodb.org/manual/reference/method/db.shutdownServer/ "Mongo docs: db.shutdownServer() command")` command
4. Do maintenance; there are several paths but the most important thing is that during maintenance **the node must not be started with production parameters**. This means a different port, but also that it shouldn't get the `[--replSet](http://docs.mongodb.org/manual/reference/program/mongod/#cmdoption--replSet "Mongo docs: --replSet command option")` parameter.
...
This video seemed like another loose end to me, but there was a blog plug attached on [how to simulate a rollback](http://comerford.cc/wordpress/2012/05/28/simulating-rollback-on-mongodb/ "Simulating Rollback on MongoDB") and the method was still relevant for version 2.6.  In the video a different approach was used, because it's possible to [create a `ReplSetTest`](http://www.mongodb.org/about/contributors/tutorial/write-tests-for-server-code/#spawning-new-instances-inside-a-js-test "Spawning new instances inside a JS Test") in the Mongo shell, which has some handy tricks to speed up the setup of various replica set configurations.
